<template>
  <div class="my-baby-page">
    ...
    >
      <div class="baby-info">
        <div class="baby-sex">
          <img src="@/static/personal/baby/girl.png" alt v-if="parseInt(item.sex) === 2" />
          <img src="@/static/personal/baby/boy.png" alt v-if="parseInt(item.sex) === 1" />
        </div>
        <div class="baby-basic">
          <div class="baby-name">{{ item.name }}</div>
          <div class="baby-birthday">{{ '出生日期：' + item.birthDay }}</div>
        </div>
      </div>
      <div class="baby-age">{{ ageDiffCount(item.birthDay) }}</div>
    </div>
    <div class="add-btn" @click="addBabyInfo" hover-class="button-hover">添加</div>
  </div>
</template>
