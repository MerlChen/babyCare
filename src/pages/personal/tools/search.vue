 ...
 * @LastEditors  : Louis
 * @LastEditTime : 2020-02-02 23:55:06
 * @Description: 修复计算BUG
 -->
<template>
  <div class="container" ref="container">
    ...
    <div class="title">2019年儿童身高体重标准快速筛选器</div>
    <div class="form-container">
      <form>
        <div class="form-item" v-if="babyList.length > 1">
          <div class="uni-list-cell-left">宝宝</div>
          <div class="uni-list-cell-db">
            <picker @change="changeBaby" :value="id" range-key="name" :range="babyList">
              <div class="uni-input">{{ healthForm.babyInfo.name }}</div>
            </picker>
          </div>
        </div>
        <div class="form-item" v-if="healthForm.babyInfo.id === '1'">
          <div class="uni-list-cell-left">月龄</div>
          <div class="uni-list-cell-db">
            <picker @change="changeAge" :value="id" range-key="name" :range="ageList">
              <div class="uni-input">{{ healthForm.age.name }}</div>
            </picker>
          </div>
        </div>
        <div class="form-item" v-if="healthForm.babyInfo.id === '1'">
          <div class="uni-list-cell-left">性别</div>
          <div class="uni-list-cell-db">
            <picker @change="changeSex" :value="id" range-key="name" :range="sexList">
              <div class="uni-input">{{ healthForm.sex.name }}</div>
            </picker>
          </div>
        </div>
        <div class="form-item">
          <div class="uni-list-cell-left">身高</div>
          <div class="uni-list-cell-db">
            <input
              class="uni-input"
              type="digit"
        ...
        <div class="form-item">
          <div class="uni-list-cell-left">体重</div>
          <div class="uni-list-cell-db">
            <input
              class="uni-input"
              type="digit"
        ...
        <div class="table-list" v-if="tableData[0].minusOne !== 0">
          <div class="table">
            <div class="thead">
              <div class="td">数值</div>
            ...
            <scroll-div scroll-y="true" height="200rpx">
              <div class="tbody">
                <div class="tr">
                ...
                <div class="tr">
                  ...
                  >{{ tableData[0].plusThree }}</div>
                  <div
                    class="td"
                    :class="{'active': parseFloat(tableData[1].plusThree) <= parseFloat(healthForm.weight)}"
                  >{{ tableData[1].plusThree }}</div>
                </div>
              </div>
            </scroll-div>
          </div>
        </div>
        <button type="primary" class="query" @click="queryResult" hover-class="active">查询</button>
      </form>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    changeBaby(res) {
      ...
      if (this.healthForm.babyInfo.id === "1") {
        ...
        this.healthForm.age.id = bestAgeMatch(
          this.ageList,
          this.healthForm.babyInfo.birthDay
        );
    ...
    async queryResult() {
      ...
      let result = await _this.$ajax.post("/api/getHealthInfo", {
        ...
        userId: wx.getStorageSync("userId"),
        babyId:
          _this.healthForm.babyInfo.id !== "1"
            ? _this.healthForm.babyInfo.id
            : "",
